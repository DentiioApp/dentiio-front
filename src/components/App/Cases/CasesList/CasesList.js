...
const CasesList = () => {
  ...
  return (
    <>
      ...
      >
        <center>
          <h1 style={{ color: Palette.primary }}>Cas cliniques</h1>
        </center>
        <br />
        {/* <Paginator 
                    pages={pages} 
          ...
          {cases &&
            cases.map((oCase, index) => {
              ...
              if (favorites.length > 0) {
                favorites.map((item) => {
                  var slashIndex =
                    item.clinicalCaseOmnipratiqueId !== undefined
                      ? item.clinicalCaseOmnipratiqueId?.lastIndexOf("/")
                      : false;
                  var caseId = slashIndex
                    ? Number(
                        item.clinicalCaseOmnipratiqueId
                          .substr(slashIndex)
                          .substr(1, slashIndex.length)
                      )
                    : item.User?.id;
                  if (caseId === oCase.id) {
                    isFavorite = true;
              ...
              return (
                <CasesItem key={index} item={oCase} favorite={isFavorite} />
              );
            })}
        </div>
        {/*
